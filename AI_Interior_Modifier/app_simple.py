...
# Simple mapping
COCO_TO_DOMAIN = {
    "sofa": "sofa", "couch": "sofa", "chair": "chair", 
    "dining table": "table", "bed": "bed", "table": "table", "tv": "tv"
}
...
def main():
    ...
    if uploaded_file:
            ...
            try:
                ...
                if detections:
                    ...
                    for i, (det, obj) in enumerate(zip(detections, objects)):
                        ...
                        st.write(f"🪑 {domain_label.capitalize()} (Confidence: {det['confidence']:.1f})")
        ...
        for i, obj in enumerate(objects):
            st.write(f"🪑 {obj['type'].capitalize()}")
            size_col1, size_col2 = st.columns(2)
            with size_col1:
                obj["w"] = st.number_input("Width (cm)", min_value=20, max_value=room_width, 
