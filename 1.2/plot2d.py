...
def plot_layout(room_dims, layout, save_path=None, save_buffer=None):
    ...
    for obj in architectural_objs:
        ...
        if label_l == "window":
            ...
            # External label above window to guarantee visibility
            ax.text(x + w/2.0, max(0, y - 6), "WINDOW",
                    ha="center", va="bottom", fontsize=9, color="#0a3d91",
                    bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor="#a8c3f2", alpha=0.9),
                    zorder=10)
        # Doors: draw swing arc and leaf
            ...
            # External label for door
            ax.text(x + w/2.0, max(0, y - 6), "DOOR",
                    ha="center", va="bottom", fontsize=9, color="#5a3a1a",
                    bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor="#d2b48c", alpha=0.9),
                    zorder=10)
        ...
        # Minimal internal label as secondary (kept small)
        txt = f"{label.upper()}"
        ax.text(x + w / 2.0, y + h / 2.0, txt,
                ha="center", va="center",
                fontsize=7.5, color="#253858",
    ...
    # Simple legend for architectural vs furniture
    try:
        legend_y = - (inset + 46)
        # Windows
        ax.add_patch(patches.Rectangle((6, legend_y), 16, 10, facecolor="#b7d8ff", edgecolor="#0066cc"))
        ax.text(26, legend_y + 9, "Windows", va="center", fontsize=9, color="#0a3d91")
        # Doors
        ax.add_patch(patches.Rectangle((96, legend_y), 16, 10, facecolor="#e0c3a0", edgecolor="#8b5a2b"))
        ax.text(116, legend_y + 9, "Doors", va="center", fontsize=9, color="#5a3a1a")
    except Exception:
        pass
