            ...
            if len(approx) == 4:
                ...
                if (0.3 < aspect_ratio < 5.0 and  # Even wider for doors/mirrors
                    ...
                    if center_y < img_h * 0.95:
                        ...
                        # Priority 5: PAINTING (very conservative to reduce false positives)
                        # Only classify as painting if strong texture AND significant edge content
                        ...
                        if crop_gray_for_paint is not None:
                            ...
                            e_p = cv2.Canny(crop_gray_for_paint, 50, 150)
                            edge_density_p = float(np.count_nonzero(e_p)) / float(e_p.size)
                            # Extremely bright and uniform -> likely wall highlight, not a painting
                            if mean_i > 190 and std_i < 10:
                                continue
                            # Require both sufficient texture and edges
                            if std_i > 40 and edge_density_p > 0.08:
                                paintings.append({
                                    "type": "painting",
                                    "bbox": [x1, y1, x2, y2],
                                    "confidence": 0.8,
    ...
    def multi_pass_detection(self, image_path: str, confidence_levels: List[float] = None) -> Dict:
        ...
        for conf_level in confidence_levels:
            ...
            for det in detections:
                # Fix: Handle missing 'label' field gracefully
                label = det.get('label', 'unknown')
                det_key = f"{label}_{int(det['bbox'][0])}_{int(det['bbox'][1])}"
        ...
        for i, painting in enumerate(paintings):
            ...
            if det_key not in all_detections:
                ...
                # Suppress generic 'painting' to avoid false positives in empty rooms; keep doors/windows/mirrors
                if lbl == "painting":
                    continue
                all_detections[det_key] = {
                    ...
                    "bbox": painting["bbox"],
                    "category": "architectural" if lbl in {"window", "door"} else "other",
