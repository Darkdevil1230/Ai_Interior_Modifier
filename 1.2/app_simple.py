...
def draw_enhanced_overlay(image, detections):
    ...
    for det in detections:
        # prefer explicit type if label missing
        raw_label = det.get("label") or det.get("type") or "object"
        is_arch = (det.get("category") == "architectural") or (isinstance(raw_label, str) and raw_label.lower() in architectural_elements)
        if is_arch:
    ...
    for det in arch_dets:
        ...
        label = det.get("label") or det.get("type") or "object"
...
def main():
    ...
    if uploaded_file:
        ...
        # Persist uploaded image bytes for possible fallback detection later
        try:
            buf = BytesIO()
            image.save(buf, format="PNG")
            st.session_state.uploaded_image_bytes = buf.getvalue()
            st.session_state.uploaded_image_size = image.size
        except Exception:
                ...
                try:
                    ...
                    if detections:
                        ...
                        if show_overlay:
                            ...
                            st.image(overlay_img, caption="🔍 AI Detection Results - Blue=Fixed, Green=Furniture", use_container_width=True)
        ...
        if objects:
            ...
            try:
                ...
                if not layouts:
                    ...
                    layouts = [layout]
                    scores = [float(optimizer.fitness(layout) + optimizer.compute_architectural_score(layout))]
    ...
    if st.session_state.optimized_layouts is not None:
        ...
        for idx, (tab, layout) in enumerate(zip(tabs, layouts)):
            with tab:
                ...
                architectural_elems = st.session_state.get('architectural_elements', [])
                # Fallback: if none stored, re-run lightweight architectural detection on the uploaded image
                if not architectural_elems and st.session_state.get('uploaded_image_bytes'):
                    try:
                        tmpf = tempfile.NamedTemporaryFile(suffix=".png", delete=False)
                        tmpf.write(st.session_state.uploaded_image_bytes)
                        tmpf.flush()
                        # Force architectural detection
                        arch_dets = enhance_detections_with_architecture(
                            yolo_detections=[], image_path=tmpf.name, force_architectural=True
                        )
                        # Parse into room coordinates
                        rd = RoomDetector()
                        img_w, img_h = st.session_state.get('uploaded_image_size', (stored_room_width, stored_room_height))
                        parsed_arch = rd.parse_detections(
                            arch_dets, CLASS_MAP, (stored_room_width, stored_room_height), (img_h, img_w)
                        )
                        # Keep only architectural
                        parsed_arch = [o for o in parsed_arch if o.get('category') == 'architectural' or o.get('type') in ["window", "door", "wall"]]
                        st.session_state.architectural_elements = parsed_arch
                        architectural_elems = parsed_arch
                    except Exception:
                        pass
                    finally:
                        try:
