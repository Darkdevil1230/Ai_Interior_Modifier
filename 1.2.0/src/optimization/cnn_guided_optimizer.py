...
class CNNGuidanceSystem:
    ...
    def calculate_architectural_fitness(self, furniture: Dict, position: Tuple[float, float]) -> float:
        ...
        if furniture_type in ["chair", "desk", "table"]:
            for window in [d for d in self.architectural_elements if d["type"] == "window"]:
        ...
        for door in [d for d in self.architectural_elements if d["type"] == "door"]:
...
class CNNGuidedOptimizer:
    ...
    def _calculate_lighting_optimization(self, layout: List[Dict]) -> float:
        ...
        score = 0.0
        windows = [d for d in self.room_analysis.get("detections", []) if d["type"] == "window"]
    ...
    def _cnn_guided_initialization(self) -> List[Dict]:
        """Initialize layout using CNN guidance."""
        layout = []
        placed_objects = set()
