...
class RoomDetector:
    ...
    def detect(self, image_path: str, conf_threshold: float = 0.35) -> List[Dict]:
        ...
        # CRITICAL FIX: Lower threshold for architectural elements (doors, windows, mirrors)
        # These are hard to detect but important - use separate threshold
        arch_threshold = max(0.1, conf_threshold * 0.5)  # Much lower for architectural elements
        
        filtered_detections = []
        for d in detections:
            # Apply lower threshold for architectural elements
            if d.get("category") == "architectural":
                if d["confidence"] >= arch_threshold:
                    filtered_detections.append(d)
            else:
                # Normal threshold for furniture
        ...
        print(f"[Detection] Found {len(detections)} objects total:")
